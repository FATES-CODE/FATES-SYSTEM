...
Google Drive에 저장된 보안 교육/주의사항 이미지 4종을 자동으로 다운로드하여 인라인(CID) 이미지로 첨부하고, 일어/한국어 쌍방향 안내 본문과 함께 Gmail 임시보관함(Draft)을 자동 생성합니다.

### 1.2 주요 기능
1. **Google Drive 보안 이미지 동적 다운로드**: GCP Service Account 도메인 위임 권한을 통해 드라이브 내 4개 이미지(`image1Id` ~ `image4Id`)를 메모리로 다운로드.
2. **CID 인라인 미디어 매핑 (MIME `related`)**: 메일 본문 내 `<img src="cid:image1">` 구조로 이미지를 직접 포함시켜 수신자의 외부 이미지 차단에 대처.
3. **동적 날짜 매핑 및 제목 구성**: 메일 생성 시점의 날짜(`yyyy.MM.dd`)를 추출하여 `<INTERNAL> PC 및 자료 관리에 대한 정기 안내 (yyyy.MM.dd)` 형태의 메일 제목 자동 부여.
4. **다중 수신자/참조자 자동 설정**: 수신자(`yjchoi@fatesinc.com`) 및 15개 영업/운송/계정 부서 참조자(CC) 자동 매핑.
5. **웹 브라우저 미리보기 및 REST API**: `GET /api/v1/it-notice/preview`를 통해 생성될 HTML 디자인을 즉시 확인하고, `POST /api/v1/it-notice/draft`를 통해 Gmail 임시보관함 생성.
...
```mermaid
flowchart TD
    subgraph TriggerLayer ["요청 계층 (Controller)"]
...
(Domain-Wide Delegation)"]
    ...
    ItNoticeService -->|3. Gmail Draft 생성| Gmail
```
...
### 3.1 `ItNoticeEmailService`
- **위치**: `com.example.fates_system.service.ItNoticeEmailService`
- **주요 기능**:
  - `createItNoticeDraft()`: Google Drive 파일 다운로드 -> `MimeMultipart("related")` 생성 -> `Gmail.users().drafts().create()` 호출.
  - `downloadDriveFile(fileId)`: `GoogleAuthService.getDriveClient()`를 통해 지정된 ID의 파일 바이너리 스트림 읽기.
  - `addImagePart()`: `ByteArrayDataSource` 및 `Header("Content-ID", "<imageX>")`를 활용한 인라인 비트맵 바이트 처리.
  - `buildHtmlBody()`: 일본어/한국어 사내 보안 안내문, 캘린더 주의사항, PC 바탕화면 및 파일 다운로드 지양, Windows 업데이트 권장 내용 및 CID 이미지 태그 HTML 생성.

### 3.2 `ItNoticeController`
...
fates:
  it-notice:
    ...
    image4-id: "1-J6-yU2Q-nI6t3U7eT0g34dE6aH6tE_"
```
